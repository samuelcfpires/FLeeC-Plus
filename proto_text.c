...
void complete_nread_ascii(conn *c) {
    ...
    if (!is_valid) {
      ...
      recl_set_curr_item_announcements_connection(c);
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas, bool should_touch) {
    ...
    do {
        while(key_token->length != 0) {
            ...
            it = limited_get(key, nkey, c->thread, exptime, should_touch, DO_UPDATE);
...
stop:
    ...
    */
    ...
    recl_check_system_item_reclamation();
}
...
static void process_meta_command(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    item *it = limited_get(key, nkey, c->thread, 0, false, DONT_UPDATE);
    if (it) {
        mc_resp *resp = c->resp;
        size_t total = 0;
        size_t ret;
        ...
        ret = snprintf(resp->wbuf + total, WRITE_BUFFER_SIZE - (it->nkey + 12),
                ...
                "exp=%d la=%llu cas=%llu fetch=%s size=%lu\r\n",
...
static void process_mget_command(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    uint64_t hv; // cached hash value for unlocking an item.
    bool failed = false;
    ...
    if (!of.locked) {
        it = limited_get(key, nkey, c->thread, 0, false, !of.no_update);
    } else {
        // If we had to lock the item, we're doing our own bump later.
        it = limited_get_locked(key, nkey, c->thread, DONT_UPDATE, &hv);
    }
    ...
    if (it == NULL && of.vivify) {
        ...
        if (it != NULL) {
            ...
            item* it_found = do_item_link(it, hv);
            if (it_found) {
                item_free(it);
                it = it_found;
            } else {
...
static void process_debugitem_command(conn *c, token_t *tokens, const size_t ntokens) {
    fprintf(stderr, "No debugitem commands implemented. Previous ones were deprecated.\n");
    out_string(c, "ERROR");
}
#endif
