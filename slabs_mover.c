...
static atomic_bool slab_reassign_running = false;
...
//            if (false/*(a->hold_lock = item_trylock(a->hv)) == NULL*/) {
//                status = MOVE_LOCKED;
//            } else {
...
static int slab_rebalance_move(struct slab_rebal_thread *t) {
    ...
    // Some stuff from memcached that was removed from fleec (item_try_lock/unlock and do_item_remove)
    // was commented so that it compiles because item lock no longer exists.
    // Slab rebalancing should not be used until rewritten.
    fprintf(stderr, "Slab reassignment not yet implemented.\n");
    exit(EXIT_FAILURE);
    ...
    if (t->rebal.completed[offset] == 0) {
        ...
        int status = 0; //!! slabs_locked_callback(_slabs_locked_cb, &cbarg);
    ...
    if (t->rebal.slab_pos >= t->rebal.slab_end) {
        ...
        if (t->rebal.busy_items) {
            ...
            if (__glibc_unlikely(ALOAD(stats.slab.slab_stats_reset_flag))) {
                memset(&stats.slab, 0, sizeof(stats.slab));
                ARSTORE(stats.slab.slab_stats_reset_flag, false);
            }
            stats.slab.slab_reassign_busy_items += t->rebal.busy_items;
...
static void slab_rebalance_finish(struct slab_rebal_thread *t) {
    ...
    stats.slab.slabs_moved++;
    stats.slab.slab_reassign_rescues += t->rebal.rescues;
    stats.slab.slab_reassign_inline_reclaim += t->rebal.inline_reclaim;
    stats.slab.slab_reassign_chunk_rescues += t->rebal.chunk_rescues;
    stats.slab.slab_reassign_busy_deletes += t->rebal.busy_deletes;
    stats.slab.slab_reassign_busy_nomem += t->rebal.busy_nomem;
...
bool is_slab_reassignment_running(void) {
    return slab_reassign_running;
}

#else

static int make_iso_compiler_happy __attribute__((unused));
