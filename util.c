...
static long _l1_dcache_line_size;

long get_l1_dcache_line_size(void) {
    if (_l1_dcache_line_size == 0) {
        _l1_dcache_line_size = sysconf(_SC_LEVEL1_DCACHE_LINESIZE);
        if (_l1_dcache_line_size < 0) {
            _l1_dcache_line_size = L1_DCACHE_LINE_SIZE_DEFAULT;
        }
    }

    return _l1_dcache_line_size;
}

void *l1_dcache_line_aligned_alloc(size_t size) {
    void *p = NULL;
    int res = posix_memalign(&p, get_l1_dcache_line_size(), size);
    if (res != 0) {
        return malloc(size);
    }
    return p;
...
void *l1_dcache_line_aligned_calloc(size_t nmemb, size_t size) {
    ...
    void *p = l1_dcache_line_aligned_alloc(totalsize);
    if (p != NULL) {
