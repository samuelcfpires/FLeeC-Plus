...
static logger_watcher* watchers[WATCHER_LIMIT]; // atomic elements
struct pollfd watchers_pollfds[WATCHER_LIMIT];
/*atomic*/ int watcher_count = 0;
/*atomic*/ bool new_watcher_added = false;
...
uint64_t logger_get_gid(void) {
    return ATOMIC_INCF(logger_gid);
}

void logger_set_gid(uint64_t gid) {
    ASTORE(logger_gid, gid);
}
...
static void logger_link_q(logger *l) {
    ...
    logger *head = logger_stack_head;
    do {
        l->next = head;
    } while (!__atomic_compare_exchange_n(&logger_stack_head, &head, l,
...
static void logger_set_flags(void) {
    ...
    for (x = 0; x < WATCHER_LIMIT; x++) {
        logger_watcher *w = AALOAD(watchers[x]);
        if (w == NULL)
            continue;
...
static void logger_thread_write_entry(logentry *e, struct logger_stats *ls,
    ...
    for (x = 0; x < WATCHER_LIMIT; x++) {
        ...
        while (!w->failed_flush &&
                (skip_scr = (char *) LFBB_WriteAcquire(w->buf, scratch_len + 128)) == NULL) {
        ...
        if (w->skipped > 0) {
            ...
            LFBB_WriteRelease(w->buf, total);
            w->skipped = 0;
        }
        /* Can't fail because LFBB_WriteAcquire succeeded. */
        memcpy(skip_scr, scratch, scratch_len);
        LFBB_WriteRelease(w->buf, scratch_len);
        ls->watcher_sent++;
    }
}
...
static int logger_thread_read(logger *l, struct logger_stats *ls) {
    ...
    data = LFBB_ReadAcquire(l->buf, &size);
    if (data == NULL) {
        return 0;
    }
    ...
    /* parse buffer */
    while (pos < size && ALOAD(watcher_count) > 0) {
...
static int logger_thread_poll_watchers(int force_poll, int watcher) {
    ...
    while (ALOAD(new_watcher_added)) {
        ASTORE(new_watcher_added, false);
        logger_set_flags();
    }
    ...
    for (x = 0; x < WATCHER_LIMIT; x++) {
        ...
        if (LFBB_HasReadAvailable(w->buf)) {
    ...
    for (x = 0; x < WATCHER_LIMIT; x++) {
        ...
        if (LFBB_HasReadAvailable(w->buf)) {
            if (watchers_pollfds[nfd].revents & (POLLHUP|POLLERR)) {
                ...
                data = LFBB_ReadAcquire(w->buf, &data_size);

                /* We can write a bit. */
                switch (w->t) {
                    case LOGGER_WATCHER_STDERR:
                        total = fwrite(data, 1, data_size, stderr);
                        LFBB_ReadRelease(w->buf, total);
                        break;
                    case LOGGER_WATCHER_CLIENT:
                        total = ((conn*)w->c)->write(w->c, data, data_size);
                        LFBB_ReadRelease(w->buf, (total == -1) ? 0 : total);
                        break;
                }

                L_DEBUG("LOGGER: poll() wrote %d to %d (data_size: %d)\n", total, w->sfd, data_size);
                if (total == -1) {
...
static void logger_thread_flush_stats(struct logger_stats *ls) {
    if (__glibc_unlikely(ALOAD(stats.log.log_stats_reset_flag))) {
        memset(&stats.log, 0, sizeof(stats.log));
        ARSTORE(stats.log.log_stats_reset_flag, false);
    }
    stats.log.log_watcher_skipped += ls->watcher_skipped;
    stats.log.log_watcher_sent    += ls->watcher_sent;
    stats_state.log.log_watchers   = ALOAD(watcher_count);
}
...
logger *logger_create(void) {
    ...
    l->buf = malloc(sizeof(LFBB_Inst_Type));
    if (l->buf == NULL) {
        free(l);
        return NULL;
    }

    void *data = malloc(settings.logger_buf_size);
    if (data == NULL) {
        free(l->buf);
        free(l);
        return NULL;
    ...
    LFBB_Init(l->buf, data, settings.logger_buf_size);
...
enum logger_ret_type logger_log(logger *l, const enum log_entry_type event, const void *entry, ...) {
    ...
    e = (logentry *) LFBB_WriteAcquire(buf, (sizeof(logentry) + reqlen));
    if (e == NULL) {
        l->dropped++;
    ...
    LFBB_WriteRelease(buf, sizeof(logentry) + e->size + e->pad);
    l->written++;
...
enum logger_add_watcher_ret logger_add_watcher(void *c, const int sfd, uint16_t f) {
    ...
    w->buf = malloc(sizeof(LFBB_Inst_Type));
    if (w->buf == NULL) {
        free(w);
        return LOGGER_ADD_WATCHER_FAILED;
    }

    void *data = malloc(settings.logger_watcher_buf_size);
    if (data == NULL) {
        free(w->buf);
        free(w);
        return LOGGER_ADD_WATCHER_FAILED;
    ...
    LFBB_Init(w->buf, data, settings.logger_watcher_buf_size);
    void *ptr = LFBB_WriteAcquire(w->buf, 4);
    if (ptr != NULL) {
        memcpy(ptr, "OK\r\n", 4);
    }
    LFBB_WriteRelease(w->buf, 4);

    ARSTORE(watchers[x], w);

    ASTORE(new_watcher_added, true);

    if (ATOMIC_FINC(watcher_count) == 0 && pthread_mutex_trylock(&logger_stack_lock) == 0) {
        pthread_cond_signal(&logger_stack_cond);
        pthread_mutex_unlock(&logger_stack_lock);
    }
