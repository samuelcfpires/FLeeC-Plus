 ...
 */
static /*atomic*/ bool allow_new_conns = true;
...
static void maxconns_handler(const evutil_socket_t fd, const short which, void *arg) {
    ...
    if (fd == -42 || ALOAD(allow_new_conns) == false) {
...
static void stats_init(void) {
    ...
    stats_state.main.accepting_conns = true; /* assuming we start in this state. */
...
void stats_reset(void) {
    ASTORE(stats.resetting_stats, true);
    //stats_prefix_clear();
    ASTORE(stats.main.main_stats_reset_flag, true);
    ASTORE(stats.slab.slab_stats_reset_flag, true);
    ASTORE(stats.log.log_stats_reset_flag, true);
    threadlocal_stats_reset();
    ASTORE(stats.resetting_stats, false);
}
...
static void settings_init(void) {
    ...
    settings.factor = 1;
    settings.chunk_size = 4 + 1 + 4 + 2 + 0 + sizeof(uint64_t); /* nkey + 1 + nval + 2 + nflags + cas */
#else
...
#endif
#ifdef ALLOCATION_CLASS_DISTRIBUTION
    settings.allocation_class_distribution_nclasses = 63;
#endif
    settings.assoc_arena_size_power = ARENA_SIZE_POWER_DEFAULT;
    settings.oom_recl_usleep = OOM_RECL_USLEEP;
    settings.oom_recl_ntries = OOM_RECL_NTRIES;
    settings.clock_frequency_power = CLOCK_FREQUENCY_POWER_DEFAULT;
}
...
#define CONN_NEW_STAT_STATE_INC(stat) \
    do { \
        if (gt_thread == NULL) { \
            /* is the main thread */ \
            stats_state.main.stat++; \
        } else { \
            t_stats_state.stat++; \
        } \
    } while (0)
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (NULL == c) {
        if (!(c = (conn *)calloc(1, sizeof(conn)))) {
            CONN_NEW_STAT_INC(malloc_fails);
        ...
        CONN_NEW_STAT_STATE_INC(conn_structs);
    ...
    ATOMIC_INC(stats_state.curr_conns);
    CONN_NEW_STAT_INC(total_conns);
...
static void conn_close(conn *c) {
    ...
    close(c->sfd);
    c->close_reason = 0;
    ASTORE(allow_new_conns, true);

    ATOMIC_DEC(stats_state.curr_conns);
 ...
 * commands. (no longer) Protected by the item lock.
 *
 * Returns the state of storage.
 */
enum store_item_type do_store_item(item *it, int comm, LIBEVENT_THREAD *t, const uint64_t hv, uint64_t *cas, bool cas_stale) {
    char *key = ITEM_key(it);
    int nkey = it->nkey;
    int nbytes = it->nbytes;
    rel_time_t exptime = it->exptime;
    enum store_item_type stored;

    switch (comm) {
        case NREAD_ADD:
            stored = !do_item_link(it, hv) ? STORED : NOT_STORED;
            break;
        case NREAD_REPLACE:
            stored = do_item_replace(it, hv) ? STORED : NOT_STORED;
            break;
        case NREAD_SET:
            do_item_set(it, hv);
            stored = STORED;
    ...
    LOGGER_LOG(t->l, LOG_MUTATIONS, LOGGER_ITEM_STORE, NULL,
            stored, comm, key, nkey, nbytes, exptime,
            /*ITEM_clsid(it)*/ 0, t->cur_sfd);
...
void server_stats(ADD_STAT add_stats, conn *c) {
    ...
    struct stats_main *stats_main = &stats.main;
    struct stats_slab *stats_slab = &stats.slab;
    struct stats_log *stats_log = &stats.log;

    struct thread_stats thread_stats;
    struct slab_stats slab_stats;
    struct thread_stats_state thread_stats_state;
    struct logger_worker_stats logger_worker_stats;

    struct stats_slab stats_slab_zero = {0};
    struct stats_main stats_main_zero = {0};
    struct stats_log stats_log_zero = {0};

    if (ALOAD(stats.resetting_stats)) {
        stats_main = &stats_main_zero;
        stats_slab = &stats_slab_zero;
        stats_log = &stats_log_zero;
        memset(&thread_stats, 0, sizeof(thread_stats));
        threadlocal_stats_aggregate(NULL, &thread_stats_state);
    } else {
        if (ALOAD(stats.main.main_stats_reset_flag))
            stats_main = &stats_main_zero;
        if (ALOAD(stats.slab.slab_stats_reset_flag))
            stats_slab = &stats_slab_zero;
        if (ALOAD(stats.log.log_stats_reset_flag))
            stats_log = &stats_log_zero;
        __atomic_thread_fence(__ATOMIC_ACQUIRE);
        threadlocal_stats_aggregate(&thread_stats, &thread_stats_state);
        slab_stats_aggregate(&thread_stats, &slab_stats);
        logger_stats_aggregate(&logger_worker_stats);
    }
    ...
    APPEND_STAT("total_connections", "%llu", (unsigned long long)(stats_main->total_conns + thread_stats.total_conns));
    if (settings.maxconns_fast) {
        APPEND_STAT("rejected_connections", "%llu", (unsigned long long)stats_main->rejected_conns);
    }
    APPEND_STAT("connection_structures", "%u", (unsigned int)(stats_state.main.conn_structs + thread_stats_state.conn_structs));
    ...
    APPEND_STAT("reserved_fds", "%u", stats_state.main.reserved_fds);
#ifdef PROXY
    ...
    APPEND_STAT("accepting_conns", "%u", stats_state.main.accepting_conns);
    APPEND_STAT("listen_disabled_num", "%llu", (unsigned long long)stats_main->listen_disabled_num);
    APPEND_STAT("time_in_listen_disabled_us", "%llu", stats_main->time_in_listen_disabled_us);
    APPEND_STAT("threads", "%d", settings.num_threads);
    APPEND_STAT("conn_yields", "%llu", (unsigned long long)thread_stats.conn_yields);
    APPEND_STAT("hash_power_level", "%u", stats_state.assoc.hash_power_level);
    APPEND_STAT("hash_bytes", "%llu", (unsigned long long)stats_state.assoc.hash_bytes);
    APPEND_STAT("hash_is_expanding", "%u", stats_state.assoc.hash_is_expanding);
    if (settings.slab_reassign) {
        APPEND_STAT("slab_reassign_rescues", "%llu", stats_slab->slab_reassign_rescues);
        APPEND_STAT("slab_reassign_chunk_rescues", "%llu", stats_slab->slab_reassign_chunk_rescues);
        APPEND_STAT("slab_reassign_inline_reclaim", "%llu", stats_slab->slab_reassign_inline_reclaim);
        APPEND_STAT("slab_reassign_busy_items", "%llu", stats_slab->slab_reassign_busy_items);
        APPEND_STAT("slab_reassign_busy_deletes", "%llu", stats_slab->slab_reassign_busy_deletes);
        APPEND_STAT("slab_reassign_busy_nomem", "%llu", stats_slab->slab_reassign_busy_nomem);
        APPEND_STAT("slab_reassign_running", "%u", is_slab_reassignment_running());
        APPEND_STAT("slabs_moved", "%llu", stats_slab->slabs_moved);
    }
    APPEND_STAT("malloc_fails", "%llu", (unsigned long long)(stats_main->malloc_fails + thread_stats.malloc_fails));
    APPEND_STAT("log_worker_dropped", "%llu", (unsigned long long)logger_worker_stats.log_worker_dropped);
    APPEND_STAT("log_worker_written", "%llu", (unsigned long long)logger_worker_stats.log_worker_written);
    APPEND_STAT("log_watcher_skipped", "%llu", (unsigned long long)stats_log->log_watcher_skipped);
    APPEND_STAT("log_watcher_sent", "%llu", (unsigned long long)stats_log->log_watcher_sent);
    APPEND_STAT("log_watchers", "%llu", (unsigned long long)stats_state.log.log_watchers);
...
#endif
    APPEND_STAT("unexpected_napi_ids", "%llu", (unsigned long long)stats_main->unexpected_napi_ids);
    APPEND_STAT("round_robin_fallback", "%llu", (unsigned long long)stats_main->round_robin_fallback);
}
...
bool get_stats(const char *stat_type, int nkey, ADD_STAT add_stats, void *c) {
    ...
    if (add_stats != NULL) {
        if (!stat_type) {
            unsigned long long curr_bytes, curr_items, total_items;
            /* prepare general statistics for the engine */
            threadlocal_stats_aggregate_general(&curr_bytes, &curr_items, &total_items);
            APPEND_STAT("bytes", "%llu", curr_bytes);
            APPEND_STAT("curr_items", "%llu", curr_items);
            APPEND_STAT("total_items", "%llu", total_items);
            APPEND_STAT("slab_global_page_pool", "%u", global_page_pool_size());
...
item* limited_get_locked(const char *key, size_t nkey, LIBEVENT_THREAD *t, bool do_update, uint64_t *hv) {
    return item_get_locked(key, nkey, t, do_update, hv);
}
...
enum delta_result_type do_add_delta(LIBEVENT_THREAD *t, const char *key, const size_t nkey,
    ...
    uint32_t flags;
    FLAGS_CONV(it, flags);
    item *new_it = do_item_alloc(ITEM_key(it), it->nkey, flags, it->exptime, res + 2);
    if (new_it == 0) {
        return EOM;
    }
    memcpy(ITEM_data(new_it), buf, res);
    memcpy(ITEM_data(new_it) + res, "\r\n", 2);
    if (!item_replace(new_it, hv)) {
        // item was removed in the meanwhile
        item_free(new_it);
        return DELTA_ITEM_NOT_FOUND;
    }
...
void do_accept_new_conns(const bool do_accept) {
    ...
    if (do_accept) {
        ...
        elapsed_us =
            (maxconns_exited.tv_sec - stats.main.maxconns_entered.tv_sec) * 1000000
            + (maxconns_exited.tv_usec - stats.main.maxconns_entered.tv_usec);
        main_stats.time_in_listen_disabled_us += elapsed_us;
        stats_state.main.accepting_conns = true;
    } else {
        stats_state.main.accepting_conns = false;
        gettimeofday(&(main_stats.maxconns_entered), NULL);
        main_stats.listen_disabled_num++;
        ASTORE(allow_new_conns, false);
 ...
 */
static size_t _transmit_post(conn *c, ssize_t res) {
...
static enum transmit_result transmit(conn *c) {
    ...
    if (res >= 0) {
        ...
        __attribute__((unused))
        size_t sent_items = _transmit_post(c, res);
        recl_unprotect_dequeue_items(c, sent_items);
...
static void conn_finalize_pending_write(conn* c) {
    if (recl_unlock_state() == 0) {
    ...
    while (!stop) {
                ...
                if (c->ssl_enabled) {
                    ...
                    if (ret <= 0) {
                        ...
                        if (err == SSL_ERROR_SYSCALL || err == SSL_ERROR_SSL) {
                            ...
                            SSL_free(ssl);
                            close(sfd);
                            main_stats.ssl_handshake_errors++;
                            break;
                        }
                    }
                }
        ...
        case conn_mwrite:
            ...
            case TRANSMIT_COMPLETE:
                recl_unprotect_all_items(c);
                if (c->pending_write) {
                    conn_finalize_pending_write(c);
                } else {
                    recl_enter_quiescent();
                }
            ...
            case TRANSMIT_SOFT_ERROR:
                // Each connection should only lock once
                if (!c->pending_write) {
                    recl_lock_state();
...
static void usage(void) {
    ...
    printf("-o, --extended            comma separated list of extended options\n"
           ...
           "                          default is xxh3 hash. options: jenkins, murmur3, xxh3\n"
...
static int _mc_meta_save_cb(const char *tag, void *ctx, void *data) {
    ...
    restart_set_kv(ctx, "hashpower", "%u", stats_state.assoc.hash_power_level);
...
int main (int argc, char **argv) {
    ...
    bool preallocate __attribute__((unused)) = false;
    int maxcore = 0;
    char *username = NULL;
    char *pid_file = NULL;
    struct passwd *pw;
    ...
    bool use_slab_sizes __attribute__((unused)) = false;
...
#ifdef ALLOCATION_CLASS_DISTRIBUTION
          "G:"  /* Distribute allocation requests across slab classes.
                 * The argument specifies how many slab classes to use (between 1 and 63, inclusive).
                 */
#endif
          "J:"  /* bucket arena size power */
          "K:"  /* CLOCK frequency power */
          "Q:"  /* OOM reclaim usleep */
          "T:"  /* OOM reclaim tries */
          ;
...
#ifdef ALLOCATION_CLASS_DISTRIBUTION
        case 'G':
            settings.allocation_class_distribution_nclasses = atoi(optarg);
            if (settings.allocation_class_distribution_nclasses < 1 || settings.allocation_class_distribution_nclasses > 63) {
                fprintf(stderr, "Allocation class distribution must be between 1 and 63\n");
                return 1;
            }
            break;
#endif
        case 'J':
            settings.assoc_arena_size_power = atoi(optarg);
            if (settings.assoc_arena_size_power < 6 || settings.assoc_arena_size_power > 42) {
                fprintf(stderr, "Assoc arena size power must be between 6 and 42\n");
                return 1;
            }
            break;
        case 'K':
            settings.clock_frequency_power = atoi(optarg);
            if (settings.clock_frequency_power < 0 || settings.clock_frequency_power > 63) {
                fprintf(stderr, "Clock frequency power must be between 0 and 63\n");
                return 1;
            }
            fprintf(stderr, "WARNING: The CLOCK frequency is overridden by a mechanism that dynamically adjusts it according to the system's avilable memory.\n"
                            "To disable this mechanism, compile with NO_MEM_PRESSURE_MONITORING.\n");

            break;
        case 'Q':
            settings.oom_recl_usleep = atoi(optarg);
            if (settings.oom_recl_usleep < 0) {
                fprintf(stderr, "OOM reclaim usleep must be a positive integer\n");
                return 1;
            }
            break;
        case 'T':
            settings.oom_recl_ntries = atoi(optarg);
            if (settings.oom_recl_ntries < 0) {
                fprintf(stderr, "OOM reclaim tries must be a positive integer\n");
                return 1;
            }
            break;
    ...
    memcached_thread_init(settings.num_threads, &start_assoc_maint);
    ...
    if (settings.slab_reassign) {
        settings.slab_rebal = start_slab_maintenance_thread();
     ...
     */
    usleep(1000);
    if (stats_state.curr_conns + stats_state.main.reserved_fds >= settings.maxconns - 1) {
