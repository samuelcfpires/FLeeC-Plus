...
#ifdef SLAB_PER_CLASS_LOCKING
#define CLASS_LOCK(id)   pthread_mutex_lock(&slabclass[id].lock)
#define CLASS_UNLOCK(id) pthread_mutex_unlock(&slabclass[id].lock)
#define GLOBAL_PAGE_POOL_LOCK()   CLASS_LOCK(SLAB_GLOBAL_PAGE_POOL)
#define GLOBAL_PAGE_POOL_UNLOCK() CLASS_UNLOCK(SLAB_GLOBAL_PAGE_POOL)
#else
static pthread_mutex_t slabs_lock = PTHREAD_MUTEX_INITIALIZER;

#define CLASS_LOCK(id)   pthread_mutex_lock(&slabs_lock)
#define CLASS_UNLOCK(id) pthread_mutex_unlock(&slabs_lock)
#define GLOBAL_PAGE_POOL_LOCK()   do {} while(0)
#define GLOBAL_PAGE_POOL_UNLOCK() do {} while(0)
#endif
...
#ifdef SLAB_PER_CLASS_LOCKING
    pthread_mutex_t lock;   /* slab class lock */
#endif
} slabclass_t;
...
#ifdef SLAB_PER_CLASS_LOCKING
    pthread_mutex_init(&slabclass[0].lock, NULL);
#endif
...
#ifdef SLAB_PER_CLASS_LOCKING
        pthread_mutex_init(&slabclass[i].lock, NULL);
#endif
    }
...
static void *do_slabs_alloc(unsigned int id,
    ...
    if (p->sl_curr == 0 && flags != SLABS_ALLOC_NO_NEWPAGE) {
        GLOBAL_PAGE_POOL_LOCK();
        do_slabs_newslab(id);
        GLOBAL_PAGE_POOL_UNLOCK();
    }
...
static void do_slabs_free(void *ptr, unsigned int id) {
    ...
    if ((it->it_flags & ITEM_CHUNKED) == 0) {
        ...
        fprintf(stderr, "Chunked free not implemented.\n");
        abort();
...
void fill_slab_stats_automove(slab_stats_automove *am) {
    fprintf(stderr, "fill_slab_stats_automove not implemented.\n");
    abort();
}
...
void slabs_free(void *ptr) {
    unsigned int id = ((item *)ptr)->slabs_clsid;
    CLASS_LOCK(id);
    do_slabs_free(ptr, id);
    CLASS_UNLOCK(id);
}
...
void *slabs_peek_page(const unsigned int id, uint32_t *size, uint32_t *perslab) {
    fprintf(stderr, "slabs_peek_page not implemented.\n");
    abort();
    return NULL;
}
...
void slabs_finalize_page_move(const unsigned int sid, const unsigned int did, void *page) {
    fprintf(stderr, "slabs_finalize_page_move not implemented.\n");
    abort();
}
...
int slabs_pick_any_for_reassign(const unsigned int did) {
    fprintf(stderr, "slabs_pick_any_for_reassign not implemented.\n");
    abort();
    return -1;
}

int slabs_page_count(const unsigned int id) {
    fprintf(stderr, "slabs_page_count not implemented.\n");
    abort();
    return 0;
}
